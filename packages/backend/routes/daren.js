...
const { normalizeNumber, normalizeDarenData } = require("../utils/normalize");
...
async function routes(fastify, options) {
  ...
  fastify.get("/api/darens", async (request, reply) => {
    try {
      ...
      if (ipLocations && ipLocations.length > 0) {
        const locationArray = Array.isArray(ipLocations)
          ? ipLocations
          : [ipLocations];
  ...
  fastify.get("/api/periods", async (request, reply) => {
    try {
      ...
      // 合并并去重
      const allPeriods = [
        ...new Set([...newPeriodsArray, ...filteredOldPeriods]),
      ];
  ...
  fastify.post("/api/darens/:id/periods", async (request, reply) => {
    try {
      ...
      // 标准化期数数据中的数字字段
      const normalizedPeriodData = normalizeDarenData({
    ...
    try {
      ...
      darens.forEach((daren) => {
        ...
        if (periodData) {
          ...
          totalEngagement +=
            (periodData.likes || 0) +
            (periodData.comments || 0) +
            (periodData.collections || 0);
        }
      });
      ...
      reply.send({
        ...
        totalEngagement,
        avgEngagement:
          darens.length > 0 ? Math.round(totalEngagement / darens.length) : 0,
      });
    } catch (err) {
      reply.code(500).send(err);
    }
    ...
    try {
      ...
      if (err.name === 'ValidationError') {
        ...
        reply.code(400).send({
          ...
          message: '数据验证失败: ' + errorMessages.join(', '),
          details: err.errors
        });
      } else {
        reply.code(500).send({
          error: 'Internal Server Error',
          message: err.message || '创建达人时发生未知错误'
        });
      }
    }
  });
  ...
  fastify.put("/api/darens/:id", async (request, reply) => {
    try {
      // 标准化数据中的数字字段
      const normalizedData = normalizeDarenData(request.body);
      const daren = await Daren.findByIdAndUpdate(
        request.params.id,
        normalizedData,
        { new: true }
      );
      ...
      if (err.name === 'ValidationError') {
        ...
        reply.code(500).send({
          error: 'Internal Server Error',
          message: err.message || '更新达人时发生未知错误'
        });
      }
    }
  });
  ...
  fastify.post("/api/parse-xhs-user", async (request, reply) => {
    ...
    try {
      ...
      // 尝试解析页面中的初始状态数据
      const initialStateMatch = pageHtml.match(
      ...
      if (Object.keys(parsedInfo).length > 1) {
        // > 1 because we always have 'type'
        // 成功解析，返回统一格式
        reply.send({
          success: true,
          ...parsedInfo,
  ...
  fastify.get("/api/analytics", async (request, reply) => {
    try {
      ...
      const totalEngagement = darens.reduce(
        (sum, d) =>
          sum + (d.likes || 0) + (d.comments || 0) + (d.collections || 0),
        0
      );
      ...
      const topPerformers = darens
        ...
        .map((d) => {
          const totalEng =
            (d.likes || 0) + (d.comments || 0) + (d.collections || 0);
          const followers = parseFloat(d.followers?.replace(/[万k]/g, "")) || 1;
          const engagementRate =
            followers > 0 ? ((totalEng / followers) * 100).toFixed(2) : 0;
      ...
      const overview = {
        ...
        avgROI:
          topPerformers.length > 0
            ? Math.round(
                topPerformers.reduce((sum, p) => sum + p.roi, 0) /
  ...
  fastify.post("/api/parse-xhs-note-simple", async (request, reply) => {
    ...
    try {
      ...
      if (metaLikes || metaCollections || metaComments) {
        ...
        parsedInfo.collections = metaCollections
          ? parseInt(metaCollections)
          : 0;
        ...
        for (let i = 0; i < scripts.length; i++) {
          ...
          if (content) {
            ...
            console.log(
              `脚本 ${i + 1}: 长度=${
                content.length
              }, __INITIAL_STATE__=${hasInitialState}, interactInfo=${hasInteractInfo}`
            );
                ...
                try {
                  ...
                  if (state.note?.noteDetailMap) {
                    ...
                    if (noteIds.length > 0) {
                      ...
                      if (note) {
                        parsedInfo.title =
                          note.title || note.desc || parsedInfo.title;
                        parsedInfo.likes = note.interactInfo?.likedCount || 0;
                        parsedInfo.collections =
                          note.interactInfo?.collectedCount || 0;
                        parsedInfo.comments =
            ...
            if (!parsedInfo.success && hasInteractInfo && hasLikedCount) {
              ...
              const patterns = [
                ...
                {
                  name: "收藏",
                  regex: /"collectedCount":(\d+)/g,
                  key: "collections",
                },
                {
                  name: "评论",
                  regex: /"commentCount":(\d+)/g,
                  key: "comments",
                },
      ...
      if (!parsedInfo.success || parsedInfo.parseMethod === "basic-fallback") {
        ...
        if (!cookie) {
          ...
          parsedInfo.message =
            "未提供 Cookie，只能获取基本信息。要获取完整的点赞、收藏、评论数据，请设置有效的 Cookie。";
        }
      }

      console.log(`\n✅ 解析完成 - 方法: ${parsedInfo.parseMethod}`);
      console.log(
        `结果: 点赞=${parsedInfo.likes}, 收藏=${parsedInfo.collections}, 评论=${parsedInfo.comments}`
      );
